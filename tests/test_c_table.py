...
class TestTableCreate(TestCase):
    ...
    def test_query_insert(self):
        # Test Base Case
        self.assertEqual(Table._query_insert(False, None, None, a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2)')

        # Test ON CONFLICT DO NOTHING
        self.assertEqual(Table._query_insert(True, None, None, a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2) ON CONFLICT DO NOTHING')

        # Test ON CONFLICT DO UPDATE
        self.assertEqual(Table._query_insert(False, Table.a, None, a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2) ON CONFLICT (a) DO UPDATE SET a = EXCLUDED.a')
        ...
        # Validate returning *
        self.assertEqual(Table._query_insert(False, None, '*', a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2) RETURNING *')

        # Validate returning single column
        self.assertEqual(Table._query_insert(False, None, Table.a, a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2) RETURNING a')

        # Validate returning multiple columns
        self.assertEqual(Table._query_insert(False, None, Table._columns, a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2) RETURNING a, b')

        # Test returning on conflict
        self.assertEqual(Table._query_insert(False, Table.a, Table.a, a=3, b=4)[0], 'INSERT INTO test.table (a, b) VALUES ($1, $2) ON CONFLICT (a) \
